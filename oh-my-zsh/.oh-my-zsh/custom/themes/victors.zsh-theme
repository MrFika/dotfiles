...
# Get the first alphabetical characters of the hostname
local short_hostname=${HOST%%[^a-zA-Z]*}

# If User is root, make user name red. Else, nice green.
if [[ $UID -eq 0 ]]; then
    local user_host='%{$terminfo[bold]$fg[red]%}%n@${short_hostname} %{$reset_color%}'
    PROMPT_PROMPT=$FG[red]
else
    local user_host='%{$terminfo[bold]$FG[077]%}%n@${short_hostname} %{$reset_color%}'
